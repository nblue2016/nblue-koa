...
class Model {
  ...
  getDb (target) {
    ...
    const mapOne =
      (data) => IIf(data, () => mapFunc(target, data.toObject()), null)

    return {
      name: target,
      exec:
        (method, ... args) => exec(target, method, ... args),
      findOne: (filter) =>
        exec(target, 'retrieve', filter, { method: 'findOne' }).
        then((data) => mapOne(data)),
      create:
        (body) => exec(target, 'create', body).
        then((data) => mapOne(data))
    }
  }
