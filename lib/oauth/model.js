...
class Model {
  ...
  getDb (name) {
    ...
    // get entity from database by name
    return dc.getDatabase(name, mapFunc)
  }

  getDbs (... targets) {
    // define function for get database with entity name
    const getDbFunc = this.getDb.bind(this)

    // declare array of result
    const rt = {}

    // fetch every item in targets
    for (const target of targets) {
      rt[target] = getDbFunc(target)
    }
  ...
  mapObject (name) {
    ...
    case 'user': {
      return (obj) => {
        ...
        // remove userid from target object
  ...
  generateAccessToken (client, user, scope) {
    if (scope) {
      C.log(`client: ${JSON.stringify(client)}`)
      C.log(`scope: ${scope}`)
      this.nothing()
    }
  ...
  getAccessToken (accessToken) {
    C.log(`## step: getAccessToken: ${accessToken}`)
  ...
  validateScope (user, client, scope) {
    ...
    if (client && client.scopes) {
      // get scopes that defined in client
      const clientScopes = client.scopes

      // check scope in request
      if (scope) {
        // create a array with all scopes in request
        const reqScopes = Array.isArray(scope) ? scope : scope.split(',')

        // filter valid scopes and join to a string with ','
        const matchedScopes = reqScopes.
                                map((item) => item.trim()).
                                filter((item) => clientScopes.includes(item))

        // return result
        return matchedScopes.length > 0 ? matchedScopes.join(',') : false
      }

      // return all scopes that defined in client
  ...
  verifyScope (accessToken, scope) {
    ...
    // failed if can't find scope in token
    ...
    // get matched scopes
    const matchedScopes = accessToken.scope.
            split(',').
            map((item) => item.trim()).
            filter((item) => reqScopes.includes(item))
