  ...
  handle (request, response) {
    ...
    const gen = function *() {
      ...
      try {
        ...
        if (scope) responseTypeOpts.scope = scope
        if (state) responseTypeOpts.state = state

        switch (responseType) {
        case CodeResponseType: {
          // generate authorization code with client and user
          const code = yield this.generateAuthorizationCode(client, user, scope)

          // if code is true, append it to response type options
          if (code) responseTypeOpts.code = code

          break
        }
        case TokenResponseType: {
          // define target for betch
          const target = {
            accessToken: this.generateAccessToken(client, user, scope),
            accessTokenExpiresAt: this.getAccessTokenExpiresAt(client),
            refreshToken: this.generateAccessToken(client, user, scope),
            refreshTokenExpiresAt: this.getRefreshTokenExpiresAt(client),
            scope
          }

          // betch a target object to get result for token
          const token = yield betch(target, { $fullReturn: true })

          // if code is true, append it to response type options
          responseTypeOpts.accessToken = token.accessToken
          responseTypeOpts.accessTokenExpiresIn =
            client.accessTokenLifetime || this.accessTokenLifetime
          responseTypeOpts.tokenType = 'Bearer'

          // save current token
          yield this.saveToken(token, client, user)
        ...
        // cretae instance of response type
        const resposneInst = new ResponseType(responseTypeOpts)

        // get grant type from instance of response type
        const grantType = resposneInst.GrantType

        // check grant type for current client
        if (!client.grants.includes(grantType)) {
          throw new UnauthorizedClientError(
            `Unauthorized client grant type: ${grantType} is invalid`
          )
        }

        // build redirect uri by response type
        const redirectUri = resposneInst.buildRedirectUri(uri)
        ...
        // update response output
        this.updateResponse(response, redirectUri, responseTypeOpts.state)
