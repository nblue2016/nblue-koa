...
class OAuthController extends Controller {
  ...
  invoke (ctx, methodName, options) {
    ...
    const gen = function *() {
      try {
        ...
        yield methodFunc(oreq, ores, opts)
      } catch (err) {
        if (logger) {
          logger.error(`oauth request failed, details: ${err.message}`)
        }

        ores.statusCode = err.statusCode || 500
        ores.status = err.status || 500
        ores.code = err.code || 500
        ores.body = {
  ...
  authenticate (ctx) {
    // invoke authenticate method in oauth server
    return this.invoke(ctx, 'authenticate', this.getScopeFromContext(ctx))
  }

  // define route method of authorize
  authorize (ctx) {
    // invoke authorize method in oauth server
    return this.invoke(ctx, 'authorize')
  }

  // define route method of token
  token (ctx) {
    // invoke token method in oauth server
    return this.invoke(ctx, 'token')
  }

  getScopeFromContext (ctx) {
