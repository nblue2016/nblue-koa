...
class ScopeComponent extends Component {
  ...
  kao () {
    ...
    const respond = napp.respond.bind(napp)

    // define function to get middle ware
    const invokeMiddlewareFunc = this.invokeMiddleware.bind(this)
    ...
    return function *(next) {
      ...
      // invoke middleware
      return yield invokeMiddlewareFunc(opts).
        then(() => next()).
        catch((err) => respond({
          ctx,
          error: err
  ...
  kao2 () {
    ...
    return function (ctx, next) {
      ...
      // invoke middleware
      return invokeMiddlewareFunc(opts).
        then(() => next()).
        catch((err) => respond({
          ctx,
          error: err
  ...
  invokeMiddleware (options) {
    ...
    // define function to get scope name
    const getScopeNamesFunc = this.getScopeNames.bind(this)

    // define function to get scopes
    const getScopesFunc = this.getScopes.bind(this)

    // define function to get scopes
    const saveScopesFunc = this.saveScopes.bind(this)

    // define function to check scopes
    const checkScopesFunc = this.checkScopes.bind(this)
    ...
    const gen = function *() {
      ...
      const scopeNames = getScopeNamesFunc(opts)

      // get all scopes by name
      const scopes = yield getScopesFunc(scopeNames)
      ...
      saveScopesFunc(scopes, opts)

      // check scope with options
      if (!checkScopesFunc(scopes, checkOptions)) {
