...
class ComponentManager {
  ...
  getComponent (name, options) {
    // check for arguments
    if (!name) throw new ReferenceError('invalid application name')

    // assign options to opts
    const opts = options || {}

    // get instance of components and startup options cache
    ...
    if (!opts.name) opts.name = componentName
    ...
    if (!cache.has(componentName) || opts.new === true) {
      ...
      // save startup options to cache
      if (opts.new !== true) {
        if (opts && Object.keys.length > 0) {
          startupOpts.set(componentName, opts)
        } else if (startupOpts.has(componentName)) {
          startupOpts.remove(componentName)
        }
      }
