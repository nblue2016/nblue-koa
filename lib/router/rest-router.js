...
class RestRouter extends SuperRouter
{
  ...
  createRouter (config) {
    ...
    for (const model of schemas.Keys) {
      ...
      try {
        ...
        const router = that.superCreateRouter(config, routerOpts)
  ...
  superAppendMWs (router) {
    super.appendMWs(router)
  }
