...
class ModelRouter extends SuperRouter
{
  ...
  mgetquery (ctx, dataAdpt) {
    ...
    return dataAdpt.retrieve(filter, that.parseOptions(opts))
  }
  ...
  model (options) {
    ...
    return function *() {
      ...
      ctx.type = 'json'
      // ctx.body = JSON.stringify(schema ? schema.model : {}, null, 4)
      that.$setBody(ctx, 200, schema ? schema.model : {})
