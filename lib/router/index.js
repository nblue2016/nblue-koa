...
class RouterManager {
  ...
  getRouter (name) {
    ...
    // get index of '::' in input string
    const index = name.indexOf('::')

    // parse router name and file if the input includes '::'
    if (index >= 0) {
      ...
      // get router name from name
      const routerName = name.substring(0, index)

      // get full path of router javascript file
      const routerFile = `${base}/${name.substring(routerName.length + 2)}`
      ...
      try {
        ...
        if (logger) {
          logger.error(
            `get class of router(${routerName}::${routerFile}) failed,` +
              ` details: ${err.message}`,
            LOGGER_NAME
          )
        }
    ...
    try {
      ...
      if (logger) {
        logger.error(
          `get class of router by name (${name}) failed,` +
            ` details: ${err.message}`,
          LOGGER_NAME
        )
      }
