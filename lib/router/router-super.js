...
const KEY_OF_ROUTES = 'routes'
const KEY_OF_MERGE_ROUTES = 'mergeRoutes'
...
class SuperRouter {
  ...
  bind () {
    ...
    const router = this.createRouter(routerConfig, {})
  ...
  use (router) {
    ...
    const amgr = napp.ApplicationManager
  ...
  getRoutes (config, options) {
    ...
    const mergeRoutes = settings.get(KEY_OF_MERGE_ROUTES, null)

    if (mergeRoutes) {
      const mergeConfig = rmgm.ConfigMaps.get(mergeRoutes)

      if (mergeConfig && mergeConfig.has(KEY_OF_ROUTES)) {
        routes.merge(mergeConfig.get(KEY_OF_ROUTES))
      }
    }

    if (opts.config &&
        opts.config.has(KEY_OF_ROUTES)) {
      routes.merge(opts.config.get(KEY_OF_ROUTES))
    }

    if (config.has(KEY_OF_ROUTES)) {
      routes.merge(config.get(KEY_OF_ROUTES))
    }
  ...
  $ok (ctx, body, options) {
    ...
    jsonApp.setHeaders(ctx, that.WebSettings)
