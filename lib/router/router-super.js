...
class SuperRouter {
  ...
  createRouter (config, options) {
    ...
    const router = this.createRouterInstance(opts)
    ...
    for (const [reqMethod, items] of routes) {
      ...
      for (const [path, method] of items) {
        if (logger) {
          logger.verbose(
            `${reqMethod.toUpperCase()} :: ${path} <=> ${method} was bind`,
            LOGGER_NAME
          )
        }
        ...
        try {
          ...
          router[reqMethod](path, methodFunc(method || 'disable', opts))
        } catch (err) {
          if (logger) {
            logger.error(
              `bind method:${method} to path:${path}` +
                ` for req(${reqMethod}) failed, ` +
                `details: ${err.message}`,
              LOGGER_NAME
            )
          }
  ...
  bindMiddlewares (router, config) {
    ...
    const routerConfig = config ? config : this.RouterConfig
  ...
  disable () {
    return this.generateResponse(DisabledError, { status: 403 })
  }
