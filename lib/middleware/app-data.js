...
class DataApp extends SuperApp {
    ...
    const gen = function *() {
      ...
      if (files.length > 0) {
        ...
        for (const file of files) {
          ...
          if (logger) {
            logger.verbose(`found schema file: ${file}.`, LOGGER_NAME)
          }
        }
        ...
        try {
          ...
          if (logger) logger.verbose('all files were parsed.', LOGGER_NAME)
        } catch (err) {
  ...
  registerConnectionString (name, cs) {
    ...
    if (logger) {
      logger.verbose(`register connection string for ${name}`, LOGGER_NAME)
    }
  }
