...
class ScopeApp extends ModelApp
{
  ...
  koa () {
    ...
    return function *(next) {
      ...
      return yield that.exec(ctx, next)
    }
  }
  ...
  exec (ctx, next) {
    ...
    const jsonApp = new JsonApp(that.App)
    ...
    return co(function *() {
      ...
      try {
        ...
        if (scope$) {
          const scopes = scopeText ? (scopeText || '').split(',') : []

          const allowGuest = set$.get('allowGuest', false)
        ...
        if (jsonErr) {
          ...
          const resSettings =
            settings.has('json-response')
              ? settings.get('json-response').toObject()
              : {}

          jsonApp.throw(ctx, err, {
            number: 500,
