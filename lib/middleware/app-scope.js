    ...
    return function *() {
      try {
        ...
        if (uncachedScopes.length > 0) {
          const caller = (adapter) => co(function *() {
            const rt = yield adapter.retrieve({ name: uncachedScopes })

            rt.forEach((item) => cache.set(item.name, item))
          })

          // get checking scopes from database
          yield dataApp.execute(ctx, 'scope', caller)
        }
