...
class Application extends EventEmitter {
  ...
  routes () {
    ...
    const gen = function *() {
      ...
      yield ctmgr.parseConfigs(config.getArray('controllers'))

      // get controllers by manager
      const controllers = ctmgr.getControllerConfigs()

      // fetch every config from router manager
      for (const [name, configMap] of controllers) {
        try {
          // create control and bind it to router by config map
          yield aq.then(ctmgr.createControllerByMap(configMap))
