...
class Application extends EventEmitter {
  ...
  createConfig (options) {
    ...
    const gen = function *() {
      ...
      saveCacheFunc(Constants.KeyOfConfig, config)

      // save settings to cache
      saveCacheFunc(Constants.KeyOfSettings, settings)
  ...
  create (options) {
    ...
    return co(function *() {
      ...
      yield that.applyComponents('create', opts)
    }).
    then(
      () => that.emit(Constants.EventOfServerInitialized, that)
    )
  }
  ...
  use () {
    ...
    return co(function *() {
      ...
      const opts = { key: 'middlewares' }

      // call koa method for every component
      const mws = yield that.applyComponents(mwFuncName, opts)

      // fetch every middleware and use it
      mws.forEach((mw) => app.use(mw))
    }).
    then(
      () => that.emit(Constants.EventOfServerUsed, that)
    )
  }
  ...
  routes () {
    ...
    then(
      () => that.emit(Constants.EventOfServerRouted, that)
    )
  }
  ...
  handleSignal () {
    ...
    Constants.
      SignalOfExit.
      forEach(
        (signal) => process.on(signal, () => co(gen))
      )
  ...
  exit (code) {
    ...
    setTimeout(() => {
      // emit event for server end
      that.emit(Constants.EventOfServerStop, that)
  ...
  getRequestInfo (options) {
    ...
    sb.append(opts.url)
    sb.append(' (')
    sb.appendFormat('%s, ', opts.method || '')
    sb.appendFormat('%s, ', opts.status || '')
    if (opts.session && opts.session.id) {
      sb.appendFormat('sid:%s, ', opts.session.id || '')
    }
    sb.appendFormat(':%sms', new Date() - opts.start)
    sb.append(')')
