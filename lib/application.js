  ...
  */
  ...
  applyComponents (method, options) {
    ...
    const gen = function *() {
      ...
      // get instance of component startup options
      const startupOpts = comgr.StartupOptions
      ...
      for (const compSection of config.getArray(opts.key || 'components')) {
        // declare component name
        let componentName = null

        // create new instance of options for component method
        const compOpts = {}

        if (typeof compSection === 'string') componentName = compSection
        else if (typeof compSection === 'object') {
          // get instance of component setcion from config
          const section = compSection.toObject()

          // get component name from map
          componentName = section.name || null

          // assign options from name section to component options
          if (section.options) {
            Object.assign(compOpts, section.options)
          }
        }
        ...
        if (startupOpts.has(componentName)) {
          Object.assign(compOpts, startupOpts.get(componentName))
        }

        // get component by name
        const component = comgr.getComponent(compSection)
