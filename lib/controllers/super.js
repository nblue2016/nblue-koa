...
class Controller {
  ...
  get ControllerManager () {
    return this.N.ControllerManager
  }
  ...
  // get instance of settings for current controller
  get Settings () {
  ...
  getLogger (name) {
    ...
    const moduleName = `controller_${name || this.Name}`
  ...
  bindMiddlewares (router, options) {
    ...
    const config = opts.config ? opts.config : this.Config
  ...
  appendRoutes (router, parent, options) {
    ...
    const app = parent ? parent : this.Application
  ...
  getSetting (key, defaultVal) {
    const settings = this.AppSettings || new Map()
  ...
  createRouterMD (name, options) {
    ...
    if (method === null) {
      ...
      // if exists method by name with server type
      if (typeName && this[typeName]) {
        // get method by name with server type
        method = this[typeName].bind(this)

        // invoke the method to create a middleware
        return method()
      }
    }

    // get method by name without server
    if (method === null) {
      method = this[name]
    }
    ...
    if (method !== null &&
      ...
      // create middleware for current method
  ...
  respondSettings (ctx, response) {
    ...
    const configs = [this.AppConfig, this.Config]
