...
class Controller {
      ...
      for (const [reqMethod, items] of routes) {
        ...
        for (const [path, method] of items) {
          ...
          try {
            ...
            const routerMethod = createMWFunc(methodFunc(methodName, opts))
  ...
  createMiddleware (func, options) {
    // create middleware function for router
    const createMWFunc = Component.createMW.bind(this)

    return createMWFunc(func, options)

    /* switch (this.ServerType) {
    case 'express':
      // return middle middleware for express
      return (req, res, next) => func(req, res).then(() => next())
    case 'koa':
      // return middle middleware for koa
      return function *() {
        const ctx = this

        return yield func(ctx.request, ctx.response)
      }
    case 'koa2':
      // return middle middleware for koa2
      return (ctx) => func(ctx.request, ctx.response)
    default:
  ...
  respond (ctx, response) {
    // get instance of nblue application
    const nblue = this.NBlue
    const respond = nblue.respond

    let respondFunc = null

    switch (this.ServerType) {
    case Constants.ServerOfExpress:
      respondFunc = respond.bind(ctx.response)
      break
    case Constants.ServerOfKoa:
    case Constants.ServerOfKoa2:
      respondFunc = respond.bind(ctx)
      break
    default:
      throw new Error(`doesn't support this method for ${this.ServerType}`)
    }

    if (response) {
      if (response.body) {
        response.body = this.getResponseBody(response.body)
      } else {
...
/*  generateResponse (target, options) {
    ...
    case 'koa2':
      ...
      return function (ctx) {
        ...
        // output error message to resposne
        return sendToResponse(target, opts)
      }
    default:
      throw new Error(`not suppport for type: ${this.ServerType}`)
    }
  }
*/
/*  sendToResponse (target, options) {
    ...
    if (opts.error && !opts.status) {
      opts.status = 500
    }

    const repsond = this.NBlue.respond
    const respondFunc = repsond.bind(opts.ctx ? opts.ctx : opts.res)

    return respondFunc(opts)
  }
*/
