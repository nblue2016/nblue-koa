...
class Controller {
  ...
  createRouter (config, options) {
    ...
    for (const [reqMethod, items] of routes) {
      ...
      for (const [path, method] of items) {
        ...
        const bindInfo = `[${reqMethod}] [${path} <=> ${methodName}]`
        ...
        try {
          ...
          if (logger) {
            logger.verbose(`bind ${bindInfo} ok.`)
          }
        } catch (err) {
          // append error message to logger
          if (logger) {
            logger.error(`bind ${bindInfo} failed.`, err)
          }
  ...
  getRoutes (config, options) {
    ...
    if (mergeRouter) {
      ...
      const mergeConfig = cmgr.ConfigMaps.get(mergeRouter)
