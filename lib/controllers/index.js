...
class ControllerManager {
  ...
  createControllerByMap (controllerConfigMap) {
    ...
    // get instance of nblue applciation
    const napp = this.NApp

    // get instance of logger
    const logger = this.getLogger()

    // get setting keys of nblue application
  ...
  getController (name, controllerFile) {
    ...
    try {
      ...
      if (logger) {
        logger.verbose(`get class of controller for ${name} ok.`)
      }
      ...
      if (logger) {
        logger.error(`get class of controller for ${name} failed.`, err)
      }
