...
class ScriptContorler extends Contorler {
  ...
  runScript (options) {
    ...
    // get response options from options
    const resOpts = opts.resOpts || {}
    ...
    return co(function *() {
      ...
      try {
        ...
        const rt = yield betch$(script, ctx$, opts.body || null)

        // convert to result
        target = yield convertFunc(rt)
      } catch (err) {
        // create result error base on catched error
        target = createErrorFunc(scriptFile, err)
      } finally {
        // output error to response
        respond(target, resOpts)

        // append error to logger
        logFunc(resOpts.error, scriptFile)
      }
    })
  }
