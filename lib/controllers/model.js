  ...
  kexec (methodFunc, options) {
    ...
    return function *() {
      ...
      try {
        ...
        const func = (adapter) => methodFunc(ctx, adapter)

        // get result from execute adapter
        const rt = yield dc.pexecute(opts.model, func)

        // output body to response
        that.outputToResponse(rt, { ctx })
      } catch (err) {
        // set message for execute failed
        const message = `apply ${opts.model}/${opts.method} failed.`

        // append error to logger
        if (logger) logger.error(message, err)

        // throw error
        that.outputToResponse(new Error(message), { ctx })
      }
    }
  }
