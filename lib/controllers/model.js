...
class ModelController extends Contorler {
  ...
  getMethodByName (name) {
    const method = super.getMethodByName(name)

    return method || super.getMethodByName(`k${name}`)
  }
  ...
  model (ctx, options) {
    ...
    const dt = instance.getComponentByName('data')
    ...
    return ctx.respond(schema ? schema.model : {})
  ...
  exec (method, options) {
    ...
    // if (!opts.method) throw new ReferenceError('method')
...
/*  // get data adapter by object name
  kexec (methodFunc, options) {
    ...
    return function *() {
      ...
      try {
        ...
        that.sendToResponse(rt, { ctx })
      } catch (err) {
        // append error to logger
        if (logger) logger.error(err.message, err)

        // throw error
        that.sendToResponse(err, { ctx })
      }
    }
  }

  // get data adapter by object name
  k2exec (methodFunc, options) {
    // declare varints of instance
    const that = this
    const opts = options || {}
  ...
  eexec (methodFunc, options) {
    ...
    const respond = this.sendToResponse.bind(this)
...
*/
  ...
  k2retrieve (ctx, adapter) {
    return this.kretrieve(ctx, adapter)
  }
  ...
  k2getup (ctx, adapter) {
    return this.kgetup(ctx, adapter)
  }
  ...
  k2getdel (ctx, adapter) {
    return this.kgetdel(ctx, adapter)
  }
  ...
  // function for koa2
  k2create (ctx, adapter) {
    return this.kcreate(ctx, adapter)
  }
  ...
  // function for koa2
  k2upset (ctx, adapter) {
    return this.kupset(ctx, adapter)
  }
  ...
  // function for koa2
  k2query (ctx, adapter) {
    return this.kquery(ctx, adapter)
  }
  ...
  // function for koa2
  k2getquery (ctx, adapter) {
    return this.kgetquery(ctx, adapter)
  }
  ...
  // function for koa2
  k2aggregate (ctx, adapter) {
    return this.kaggregate(ctx, adapter)
  }
  ...
  // function for koa2
  k2count (ctx, adapter) {
    return this.kcount(ctx, adapter)
  }
  ...
  // function for koa2
  k2getcount (ctx, adapter) {
    return this.kgetcount(ctx, adapter)
  }
  ...
  // function for koa2
  k2update (ctx, adapter) {
    return this.kupdate(ctx, adapter)
  }
  ...
  // function for koa2
  k2delete (ctx, adapter) {
    return this.kdelete(ctx, adapter)
  }
