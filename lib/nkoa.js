...
const loggerMW = new LoggerMW()
const staticMW = new StaticMW()
const scopeMW = new ScopeMW()
const jsonMW = new JsonMW()
const helloMW = new HelloMW()
...
class nkoa
{
  ...
  create (configFile, options) {
    ...
    return co(function *() {
      ...
      const logger =
        config.has('logger') ? loggerMW.create(config, {}) : null

      if (ctx && logger) ctx.logger = logger

      const schemas =
        config.has('schemas') ? yield dataMW.create(that) : null
  ...
  getMiddleware (name) {
    ...
    switch (name) {
    case 'nblue-logger':
      return loggerMW.koa(that)
    case 'nblue-static':
      return staticMW.koa(that)
    case 'nblue-scope':
      return scopeMW.koa(that)
    case 'nblue-json':
      return jsonMW.koa(that)
    case 'nblue-data':
      return dataMW.koa(that)
    case 'nblue-hello':
      return helloMW.koa(that)
    default:
