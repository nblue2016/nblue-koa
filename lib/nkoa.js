...
class nkoa
{
  ...
  constructor (app) {
    ...
    // declare instance of map for inner object
    this._cache = new Map()
    this._boxes = new Map()
  }
  ...
  get Config () {
    return this.getFromCache(KeysOfCache.Config)
  }
  ...
  createApp (app, options) {
    ...
    if (app.create &&
      ...
      return aq.then(app.create(opts))
    }
  ...
  create (options) {
    ...
    return co(function *() {
      ...
      // create middlewares with definition
      yield aq.series(
        config.
          getArray('middlewares').
          map((name) => that.getApp(name)).
          filter((app) => app).
          map((app) => that.createApp(app, opts))
        )
    })
  }
