...
class nkoa {
  ...
  listen () {
    ...
    // write info to log
    if (logger) {
      logger.info(`create web services on ${port}`)
    }
    ...
    ExitSignal.forEach(
      (signal) =>
        process.on(
          signal,
          () => co(gen).
            then(() => setTimeout(() => P.exit(), 100)).
            catch(() => setTimeout(() => P.exit(1), 100))
        )
    )
  ...
  applyComponents (method, options) {
    ...
    return co(function *() {
      ...
      for (const name of config.getArray('components')) {
        ...
        const component = comgr.getComponent(componentName)
