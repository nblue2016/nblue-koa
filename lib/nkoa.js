...
const ApplicationManager = require('./components')
...
class nkoa {
  ...
  routes () {
    ...
    const cmgr = that.ControllerManager
    ...
    return co(function *() {
      ...
      yield cmgr.parseConfigs(config.getArray('routes'))

      // fetch every config from router manager
      for (const [name, configMap] of cmgr.getControllerConfigs()) {
        try {
          // create control and bind it to router by config map
          cmgr.createControllerByMap(configMap)
