...
const LoggerApp = require('./middleware/logger')
const StaticApp = require('./middleware/static')
const ScopeApp = require('./middleware/scope')
const JsonApp = require('./middleware/json')
const HelloApp = require('./middleware/hello')
const DataApp = require('./middleware/data')
...
class nkoa
{
  ...
  create (configFile, options) {
    ...
    return co(function *() {
      ...
      const logger =
        config.has('logger')
          ? new LoggerApp(app).create(config, {})
          : null
      ...
      const schemas =
        config.has('schemas')
          ? yield new DataApp(app).create(that)
          : null
  ...
  getMiddleware (name) {
    ...
    switch (name) {
    case 'nblue-logger':
      return new LoggerApp().koa(that)
    case 'nblue-static':
      return new StaticApp().koa(that)
    case 'nblue-scope': {
      const scopeApp = new ScopeApp(that.App)

      return scopeApp.createKoa()
    }
    case 'nblue-json':
      return new JsonApp().koa(that)
    case 'nblue-data':
      return new DataApp().koa(that)
    case 'nblue-hello':
      return new HelloApp().koa(that)
    default:
